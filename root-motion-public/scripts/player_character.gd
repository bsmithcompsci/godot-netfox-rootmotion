...
@export_range(0.0, 5.0) var network_update: float = 0.25
...
func _rollback_tick(delta: float, _tick: int, _is_fresh: bool) -> void:
    # Debugging the network information!
    DebugDraw2D.set_text("[%s] Network Delta" % network_id, delta)
    DebugDraw2D.set_text("[%s] Network Tick:" % network_id, _tick)
    DebugDraw2D.set_text("[%s] Network Is Fresh:" % network_id, _is_fresh)
